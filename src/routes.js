...
const AnnouncementContactsController =
    require("./Controllers/AnnouncementContacts");
const UsersController = require("./Controllers/Users");

// CHARACTERISTICS
routes
    .post("/api-imovel-finder/characteristics",
          passport.authenticate("bearer", {session : false}), celebrate({
    ...
    .get("/api-imovel-finder/characteristics",
         passport.authenticate("bearer", {session : false}),
         CharacteristicsController.getAll)
    .get("/api-imovel-finder/characteristics/:id",
         passport.authenticate("bearer", {session : false}),
         CharacteristicsController.getByID)
    .delete("/api-imovel-finder/characteristics/:id",
            passport.authenticate("bearer", {session : false}),
            CharacteristicsController.delete)
    .patch("/api-imovel-finder/characteristics/:id",
           passport.authenticate("bearer", {session : false}), celebrate({
             [Segments.BODY] : Joi.object({
               name : Joi.string(),
               active : Joi.boolean(),
...
// PROPERTY TYPES
routes
    .post("/api-imovel-finder/property-types",
          passport.authenticate("bearer", {session : false}), celebrate({
            [Segments.BODY] : Joi.object({
              name : Joi.string().required(),
    ...
    .get("/api-imovel-finder/property-types",
         passport.authenticate("bearer", {session : false}),
         PropertyTypesController.getAll)
    .get("/api-imovel-finder/property-types/:id",
         passport.authenticate("bearer", {session : false}),
         PropertyTypesController.getByID)
    .delete("/api-imovel-finder/property-types/:id",
            passport.authenticate("bearer", {session : false}),
            PropertyTypesController.delete)
    .patch("/api-imovel-finder/property-types/:id",
           passport.authenticate("bearer", {session : false}), celebrate({
             [Segments.BODY] : Joi.object({
               name : Joi.string(),
               icon : Joi.string().uri(),
...
// ANNOUNCEMENTS
routes
    .post("/api-imovel-finder/announcements",
          passport.authenticate("bearer", {session : false}), celebrate({
            [Segments.BODY] : Joi.object({
    ...
    .patch("/api-imovel-finder/announcements/:id",
           passport.authenticate("bearer", {session : false}), celebrate({
             [Segments.BODY] : Joi.object({
    ...
    .get("/api-imovel-finder/announcements/:id",
         passport.authenticate("bearer", {session : false}),
         AnnouncementsController.getByID)
    .get("/api-imovel-finder/announcements",
         passport.authenticate("bearer", {session : false}),
         AnnouncementsController.getAll)
    .delete("/api-imovel-finder/announcements/:id",
            passport.authenticate("bearer", {session : false}),
            AnnouncementsController.delete);

// ANNOUNCEMENTS CHARACTERISTICS
routes
    .post("/api-imovel-finder/announcements/:id/characteristic",
          passport.authenticate("bearer", {session : false}), celebrate({
            [Segments.BODY] : Joi.object({
...
// ANNOUNCEMENT PHOTOS
routes
    .post("/api-imovel-finder/announcements/:announcement_id/photo",
          passport.authenticate("bearer", {session : false}), celebrate({
            [Segments.BODY] : Joi.object({
...
routes
    .post("/api-imovel-finder/announcements/:announcement_id/contact",
          passport.authenticate("bearer", {session : false}), celebrate({
            [Segments.BODY] : Joi.object({
    ...
    .patch("/api-imovel-finder/announcements/:announcement_id/contact",
           passport.authenticate("bearer", {session : false}), celebrate({
             [Segments.BODY] : Joi.object({
...
// USERS
routes
    .post("/api-imovel-finder/users", celebrate({
            [Segments.BODY] : Joi.object({
              email : Joi.string().email().max(100).required(),
    ...
    .post("/api-imovel-finder/users/login",
          passport.authenticate("local", {session : false}),
          UsersController.login)
    .post("/api-imovel-finder/users/logout/:id",
          passport.authenticate("bearer", {session : false}),
          UsersController.logout)
    .post("/api-imovel-finder/users/:id/verify-email", celebrate({
            [Segments.BODY] : Joi.object({code : Joi.number().required()}),
          }),
          UsersController.updateVerifiedEmail)
    .patch("/api-imovel-finder/users/:id",
           passport.authenticate("bearer", {session : false}), celebrate({
             [Segments.BODY] : Joi.object({
               email : Joi.string().email().max(100),
    ...
    .get("/api-imovel-finder/users/:id",
         passport.authenticate("bearer", {session : false}),
